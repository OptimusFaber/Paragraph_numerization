...
class Make_tree:
    ...
    def numeral_paragraphs(self, elem):
        ...
        if not self.tree:   # in case if its first paragraph
            buf=''
            parent = self.root
            pos = 0
        ...
        for i in range(-1, -len(self.tree)-1, -1):
            if self.tree[i].sign == '.' and self.tree[i].data_type == 'number': 
                ...
                if node[:-1] == paragraph[:-1]:
                    self.tree.append(Node(elem[0], sign=elem[1], pos=elem[2], parent=self.tree[i].parent, data_type=elem[3], status='EXISTING', delimetre = elem[4]))
                    return
                ...
                if self.tree[i].sign == '.' and self.tree[i].data_type == 'number': 
                    ...
                    if node == papa[:-1] or len(papa) == 1:
                        ...
                        for j in range(-1, -len(reletives)-1, -1):
                            ...
                            if j != -len(reletives):
                                self.tree.append(Node('.'.join(reletives[j])+'.', sign='.', pos=idx, parent=parent, data_type='number', status='MISSING', delimetre = delimetre))
                                parent = self.tree[-1]
                            else:
                                self.tree.append(Node(elem[0], sign=elem[1], pos=elem[2], parent=parent, data_type=elem[3], status='EXISTING', delimetre = elem[4]))
