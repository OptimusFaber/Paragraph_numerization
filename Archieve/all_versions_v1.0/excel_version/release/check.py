...
def check_file(json_path=None, config_path=None, report_output=None, json_output=None, global_log_path=None, libre_path='libreoffice', text=False, test=False, visualize=False, add_file=None, new_format=0): 
    ...
    if paragraph_check:
        ...
        try:
            ...
            dcts = []
            data = parse(t, json_path)
            for lst in data.values():
                tree = Make_tree()
                dcts.extend(tree.walk(lst, json_path, log_path))
                if text:
                    print(lst)
                if visualize:
                    tree.show()
    ...
    if test:
        ...
        try:
            feedback2 = abb_finder(json_text=t, abbs=abb_check, add_info=add_info, json_path=json_path, log_path=log_path, new_format=new_format)
        except Exception as err:
            logging.basicConfig(filename=global_log_path, level=logging.DEBUG, 
                    ...
                    for cl in st['Cells']:
                        ...
                        if cl['Entities']:
                            for j in range(len(cl['Entities'])):
                                mistake = compare_single_text(cl['Entities'][j], log_path=report_output, txt_path=json_path)
                                ...
                                if not cl['Entities'][j]["IsSkip"]:
                                    if not cl['Entities'][j]["IsValid"]:
                                        try:
                                            report.append({"Error": cl['Entities'][j]['Errors'][0]['Type'],
