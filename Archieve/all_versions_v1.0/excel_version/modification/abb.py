...
def abb_finder(json_text, abbs=True, add_info=None, json_path=None, log_path='myapp.log', new_format=0):
            ...
            try:
                ...
                if add_info:
                    if add_info.get('Corruption'):
                        corr_fac = list(filter(lambda x: x[0] , zip([re.search(f"(^|[^0-9^A-Z^a-z^А-Я^а-я]){word}([^0-9^A-Z^a-z^А-Я^а-я]|$)", devided_text[i][0].lower()) for word in corruption_factor_set], corruption_factor_set)))
                        for cor in corr_fac:
                            sentence = "Фраза «{}»".format(cor[1])
                            feedback_list.append(["Corruption", sentence, i, cor[1]])
                    if add_info.get('No_NPA'):
                        no_conn  = list(filter(lambda x: x[0],  zip([re.search(f"(^|[^0-9^A-Z^a-z^А-Я^а-я]){word}([^0-9^A-Z^a-z^А-Я^а-я]|$)", devided_text[i][0].lower()) for word in no_connection_with_npa_set], no_connection_with_npa_set)))
                        for no in no_conn:
                            sentence = "Фраза «{}»".format(no[1])
                            feedback_list.append(["NoNPA", sentence, i, no[1]])
                    if add_info.get('IncorrectForm'):  
                        inc_frm  = list(filter(lambda x: x[0],  zip([re.search(f"(^|[^0-9^A-Z^a-z^А-Я^а-я]){word}([^0-9^A-Z^a-z^А-Я^а-я]|$)", devided_text[i][0].lower()) for word in incorrect_formulation_set], incorrect_formulation_set)))
                        for inc in inc_frm:
