...
def check_file(json_path=None, config_path=None, report_output=None, json_output=None, global_log_path=None, libre_path='libreoffice', text=False, test=False, visualize=False, add_file=None, new_format=0):    
    ...
    if config_path:
        ...
        if not new_format:
            ...
            try:
                ...
                abb_check = j["Settings"]["CheckAbbreviations"]
                try:
                    pref = {"CheckAbbreviations": "Abbreviation", "CheckCorruption":"Corruption", "CheckNoNpaConnection":"No_NPA", "CheckIncorrectSentences":"IncorrectForm"}
                    for marker, dct in list(pref.items()):
    ...
    if test:
        ...
        try:
            feedback2 = abb_finder(text=t, abbs=abb_check, dicts=None,  add_info=add_info, content_strings=content, json_path=json_path, log_path=log_path, new_format=new_format)
        except Exception as err:
            logging.basicConfig(filename=global_log_path, level=logging.DEBUG, 
            ...
            if elem == 'Paragraphs':
                for e in range(len(t[elem])):
                    if t[elem][e]['Index'] in dictionary.keys():
                        if t[elem][e].get('Errors') is None:
                            t[elem][e]['Errors'] = dictionary[t[elem][e]['Index']]
                        else:
                            # if dictionary[t[elem][e]['Index']] not in t[elem][e]['Errors']:
                                t[elem][e]['Errors'].extend(dictionary[t[elem][e]['Index']])
                        
                        # t[elem][e]['Errors'] = [i for n, i in enumerate(t[elem][e]['Errors']) if i not in t[elem][e]['Errors'][:n]]
                        ...
                        for j in range(len(t[elem][e]['Entities'])):
                            ...
                            if not t[elem][e]['Entities'][j]["IsSkip"]:
                                if not t[elem][e]['Entities'][j]["IsValid"]:
                                    try:
                                        report.append({"Error": t[elem][e]['Entities'][j]['Errors'][0]['Type'],
                        ...
                        for c in range(len(t[elem][e]['Rows'][cell]['Cells'])):
                            for g in range(len(t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"])):
                                if t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Index'] in dictionary.keys():
                                    if t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g].get('Errors') is None:
                                        t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Errors'] = dictionary[t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Index']]
                                    else:
                                        # if dictionary[t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Index']] not in t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Errors']:
                                            t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Errors'].extend(dictionary[t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Index']])
                                ...
                                if t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities']:
                                    for j in range(len(t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'])):
                                        ...
                                        if not t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'][j]["IsSkip"]:
                                            if not t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'][j]["IsValid"]:
                                                try:
                                                    report.append({"Error": t[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'][j]['Errors'][0]['Type'],
