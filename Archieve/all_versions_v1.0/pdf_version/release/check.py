...
def check_file(json_path=None, config_path=None, report_output=None, json_output=None, global_log_path=None, libre_path='libreoffice', text=False, test=False, visualize=False, new_format=0):    
    print('VERSION 1.7')
    ...
    if test:
        ...
        try:
            feedback2 = abb_finder(text=t, abbs=abb_check, add_info=add_info, json_path=json_path, log_path=log_path)
        except Exception as err:
            logging.basicConfig(filename=global_log_path, level=logging.DEBUG, 
            format=f'%(asctime)s %(levelname)s module: abb.py \nText path: {json_path}\n')
            logger=logging.getLogger(__name__)
            logger.error(err)
            sys.exit("Error with abbreviations!")
