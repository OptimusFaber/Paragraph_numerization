...
def abb_finder(text, abbs=True, add_info=None, json_path=None, log_path='myapp.log', new_format=0):
    ...
    for p in pos:
        ...
        while flag:
            buf1 = ''
            block = [item for item in text['Paragraphs'] if item['Index'] == p[0]][0]["Text"][p[1]:].split("\n")
            paragraph = p[0]
            c = index = string = 0
    ...
    for i in range(len(js)):
        ...
        for j in range(len(strings)):
            try:
                ...
                # if add_info:
                #     if add_info.get('Corruption'):
                #         corr_fac = list(filter(lambda x: x[0], zip([re.search(f"(^|[^0-9^A-Z^a-z^А-Я^а-я]){word}([^0-9^A-Z^a-z^А-Я^а-я]|$)", strings[j].lower()) for word in corruption_factor_set], corruption_factor_set)))
                #         for cor in corr_fac:
                ...
                #     if add_info.get('No_NPA'):
                #         no_conn  = list(filter(lambda x: x[0],  zip([re.search(f"(^|[^0-9^A-Z^a-z^А-Я^а-я]){word}([^0-9^A-Z^a-z^А-Я^а-я]|$)", strings[j].lower()) for word in no_connection_with_npa_set], no_connection_with_npa_set)))
                #         for no in no_conn:
                ...
                #     if add_info.get('IncorrectForm'): 
                #         inc_frm  = list(filter(lambda x: x[0],  zip([re.search(f"(^|[^0-9^A-Z^a-z^А-Я^а-я]){word}([^0-9^A-Z^a-z^А-Я^а-я]|$)", strings[j].lower()) for word in incorrect_formulation_set], incorrect_formulation_set)))
                #         for inc in inc_frm:
