    ...
    def non_text(self, elem):
        ...
        if elem[0] in self.non_txt_dct[elem[1]]:
            self.tree.append(Node(" " + elem[1] + " " + elem[0], sign=elem[1], pos=elem[2], parent=parent, data_type='None', 
                                    status='DUPLICATE', sup=elem[5], string_index = elem[4], elem_name=elem[5]))
            return
                                ...
                                for j in range(rel[e]+1, cur[e]):
                                    try:
                                        self.tree.append(Node(elem[1] + " " + '{}.1'.format(j), sign=elem[1], pos=elem[2], parent=parent, data_type=elem[3], 
                                                                status='MISSING', sup=elem[5], string_index = elem[4], elem_name=elem[1] + " " + '{}.1'.format(j)))  
                                    except:
                                        continue     
                            else:
                                for j in range(rel[e]+1, cur[e]):  
                    ...
                    for i in range(n2+1, n1):
                        ...
                        try:
                            self.tree.append(Node(elem[1] + " " + self.revfunc(i), sign=elem[1], pos=elem[2], parent=parent, data_type=elem[3], 
                                                status='MISSING', sup=elem[5], string_index = elem[4], elem_name=elem[5]))
                ...
                if self.numeral_check("1", elem[0]):
                    ...
                    if rel[0] != cur[0]:
                        self.tree.append(Node(elem[1] + " 1", sign=elem[1], pos=elem[2], parent=parent, data_type=elem[3], 
                                            status='MISSING', sup=elem[5], string_index = elem[4], elem_name=elem[1] + " 1"))
                    adress = []
                    k1 = k2 = True
                    dif = len(cur) - len(rel)
                    if len(cur) > len(rel):
