...
class Parse_numberings:
                ...
                if self.numeral_check("1", elem[0]):
                    ...
                    if rel[0] != cur[0]:
                        self.tree.append(Node(elem[1] + " 1", sign=elem[1], pos=elem[2], parent=parent, data_type=elem[3], 
                                            status='MISSING', delimetr = elem[4], sup=elem[5], elem_name=elem[1] + " 1"))
                    adress = []
                    k1 = k2 = True
                    dif = len(cur) - len(rel)
                    if len(cur) > len(rel):
    ...
    def numeral_paragraphs(self, elem):
        # В начале метода добавляем проверку на дубликаты
        for node in self.tree:
            if (node.name == elem[0] and 
                node.sign == elem[1] and 
                node.status != 'INCORRECT'):
                for x in range(10):
                    try:
                        self.tree.append(Node(x * " " + elem[0], sign=elem[1], 
                                            pos=elem[2], parent=node.parent,
                                            data_type='numbers', status='DUPLICATE',
                                            delimetr=None, sup=elem[5], 
                                            elem_name=elem[5]))
                        return
                    except:
                        continue
        
        # Далее идет существующий код метода...
        ...
        if not posible_relatives:
            ...
            if not similar_paragraphs:
                # Если это первый параграф неправильной последовательности
                for x in range(10):
                    try:
                ...
                if last_num >= last_number:
                    # Если такой номер уже есть - это дубликат
                    for x in range(10):
                        try:
                            self.tree.append(Node(x * " " + elem[0], sign=elem[1], pos=elem[2],
                    ...
                    # Добавляем пропущенные параграфы
