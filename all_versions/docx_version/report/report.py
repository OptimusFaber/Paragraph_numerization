...
class Report_generator:
    ...
    def feedback_parse(self, feedback_json, NpaNta: bool):
        report = []
        for elem in feedback_json.keys():
            if elem == 'Paragraphs':
                for e in range(len(feedback_json[elem])):
                    if feedback_json[elem][e]['Errors']:
                        for p in range(len(feedback_json[elem][e]['Errors'])):
                            report.append({"Error": feedback_json[elem][e]['Errors'][p]["Description"],
                                            "Feedback": feedback_json[elem][e]['Errors'][p]["Type"]})

                    if feedback_json[elem][e]['Entities']:
                        for j in range(len(feedback_json[elem][e]['Entities'])):
                            if not feedback_json[elem][e]['Entities'][j]["IsSkip"]:
                                if NpaNta and feedback_json[elem][e]['Entities'][j]['Status'] == 'Actual':
                                    continue
                                report.append({"Error": "Неверные сущности",
                                                "Feedback": feedback_json[elem][e]['Entities'][j]})
                        
            elif elem == 'Tables':
                for e in range(len(feedback_json[elem])):
                    for cell in range(len(feedback_json[elem][e]['Rows'])):
                        for c in range(len(feedback_json[elem][e]['Rows'][cell]['Cells'])):
                            for g in range(len(feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"])):
                                if feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Errors']:
                                    for p in range(len(feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Errors'])):
                                        report.append({"Error": feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Errors'][p]["Description"],
                                                        "Feedback": feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Errors'][p]["Type"]})
                            
                                if feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities']:
                                    for j in range(len(feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'])):
                                        if not feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'][j]["IsSkip"]:
                                            if NpaNta and feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'][j]['Status'] == 'Actual':
                                                continue
                                            report.append({"Error": "Неверные сущности",
                                                            "Feedback": feedback_json[elem][e]['Rows'][cell]['Cells'][c]["Paragraphs"][g]['Entities'][j]})
        return report