def fb(dictonaries):
    ...
    for dct in dictonaries:
        ...
        for i in range(1, len(keys)):
            if dct[keys[i]]['status'] == 'MISSING' or dct[keys[i]]['status'] == 'DUPLICATE':
                feedback_list.append([dct[keys[i]]['name'], dct[keys[i]]['sign'], dct[keys[i]]['pos'], dct[keys[i]]['delimetr'], dct[keys[i]]['data_type'], dct[keys[i]]['status'], dct[keys[i]]['sup'],dct[keys[i]]['text_index']])
    ...
    for i in range(len(feedback_list)):
        ...
        feedback_list[i][4] = feedback_list[i][7]
        feedback_list[i] = feedback_list[i][:5]
