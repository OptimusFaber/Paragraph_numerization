...
class Abb_finder(Abbreviations_processing):
    ...
    def abb_table_search(self, text: str):
        pos = []
        for elem in text['Worksheets']:
            self.parsed_text[elem['Name']]=[]
            self.parsed_text[elem['Name']].append(elem['Rows'])
            for e in elem['Rows']:
                for n in range(len(e["Cells"])):
                    string = e["Cells"][0]['Text']
                    self.all_words += re.sub("[^A-Za-zА-Яа-яЁё\s]", "", string) + " "
                    match = re.search(re.compile(r"(С|с)окращени(я|й)|(Т|т)ермин(ы|ов)", flags=re.IGNORECASE), string)
                    if match is not None:
                        pos.append(e['Cells'][n]['Address'])
        
        return pos

    @log_errors
    def abb_table_parser(self, positions):
        for pos in positions:
            flag = True
            table_name = re.sub("[!][A-Z]+[0-9]+", "", pos.replace("'", ""))
            index = self.parsed_text[table_name][0].index([i for i in self.parsed_text[table_name][0] if [j for j in i['Cells'] if j['Address'] == pos]][0])
            c = 0
            for ind in range(index, len(self.parsed_text[table_name][0])):
                if not flag:
                    break
                row = self.parsed_text[table_name][0][ind]
                for cell in row['Cells']:
                    if c == 5:
                        flag = False
                        break
                    txt = self._clean_text(cell['Text'])
    ...
    @log_errors
    def abb_extractor(self, orig_text, index, cell_address):
        ...
        for r, _ in res:
            ...
            self.feedback_list.append(["Abbreviation", sentence, cell_address, r])
    ...
    def start(self, text, abbs=True, add_info=None, defis=False, new_format=0):
        ...
        table_cords = self.abb_table_search(text)
        ...
        self.abb_table_parser(table_cords)
        ...
        self.feedback_list = []
        for lst, part in self.parsed_text.items():
            for string in part[0]:
                index, cells = string.values()
                for i in range(len(cells)):
                    cell_address = cells[i]['Address']
                    self.buf_of_added = []
                    orig_text = cells[i]['Text']
                    clean_text = self._clean_text(cells[i]['Text'])
                    ...
                    if self.buf_of_added:
                        ...
                        self.abb_extractor(orig_text, index, cell_address)
        #^--------------------------------------------------------------------------------------------------------------------
