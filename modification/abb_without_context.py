...
for i in range(len(devided_text)):
    if i not in list(abb_set.values()):
        ...
        for cor in corr_fac:
            print("CorruptionFactorError: string number - {}, error - {}".format(i+1, cor[1]))
        for no in no_conn:
            print("NoConnectionWithNPAError: string number - {}, error - {}".format(i+1, no[1]))
        for inc in inc_frm:
            print("IncorrectFormulationError: string number - {}, error - {}".format(i+1, inc[1]))
        #?------------------------------------------------------------------------------------
